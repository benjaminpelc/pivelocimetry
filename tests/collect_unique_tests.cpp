...
TEST_CASE("Bpu::collect_unique") {
  ...
  SECTION("length of uxs should be 7") { CHECK(uxs.size() == 6); }

  SECTION("first element should be 1") { CHECK(uxs.front() == 1); }

  SECTION("last element should be 7") { CHECK(uxs.back() == 7); }
}
...
TEST_CASE("Bpu::collect_unique with pred") {
  ...
  SECTION("length uxs1 should equal 4") { CHECK(uxs1.size() == 5); }

  SECTION("length uxs1 should equal 4") { CHECK(uxs2.size() == 4); }

  SECTION("length uxs3 should equal 4") { CHECK(uxs3.size() == 4); }

  SECTION("uxs3 should equal expected") { CHECK(uxs3 == ex); }
}
