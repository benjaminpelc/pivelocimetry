...
TEST_CASE( "BPU::cast_round_if_integral"){

	auto a = 3.0;
	auto b = 3.6;

	SECTION( "should equal 3" ) {
		CHECK(cast_round_if_integral<int>(a) == 3);
	}

	SECTION( "should == 3.0" ) {
		CHECK(cast_round_if_integral<double>(a) == 3.0);
	}
	...
	SECTION( "should == 3.6" ) {
		CHECK(cast_round_if_integral<double>(b) == 3.6);
	}
