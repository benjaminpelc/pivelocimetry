...
TEST_CASE("PivEng::find_ccf_peaks") {
  ...
  SECTION("First peak should be 0.998") { CHECK(p1.get_value() == 0.998); }
  ...
  SECTION("Second peak should be 0.810") { CHECK(p2.get_value() == 0.810); }
  ...
  SECTION("Third peak should be 0.321") { CHECK(p3.get_value() == 0.321); }
