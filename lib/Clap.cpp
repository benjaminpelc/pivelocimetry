...
Clap::Clap(int argc, char **argv)
    : m_flags(), m_argc(argc), m_argv(argv), m_flagArgsMap() {
  getFlags();
  std::for_each(m_flags.begin(), m_flags.end(),
                [&](auto &s) { this->m_flagArgsMap[s] = this->flagArgs(s); });
}

char **Clap::begin() const { return &m_argv[0]; };

char **Clap::end() const { return &m_argv[m_argc]; };

const std::set<std::string> &Clap::flagSet() const { return m_flags; }

const Clap::FlagArgsMap &Clap::getFlagsWithArgs() const {
  return m_flagArgsMap;
}

int Clap::numArgs() const { return m_argc; };

std::string Clap::progName() const { return m_argv[0]; };
...
bool Clap::isFlag(const std::string a) { return a[0] == '-'; }
