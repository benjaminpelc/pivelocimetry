...
private:
  ...
  sf::Color vector_color(const double vel_magnitude,
                         const double max_vel_magnitude) const;
  sf::VertexArray make_vector_graphic(PivEng::PivVector &piv_vector);
  sf::VertexArray make_axis_box(const int i_min, const int j_min,
                                const int i_max, const int j_max) const;
  sf::VertexArray make_crosshairs(const int i_min, const int j_min,
                                  const int i_max, const int j_max) const;
