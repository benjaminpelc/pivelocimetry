...
void PivPoint::printPeaks() {
  ...
  for (auto &p : m_peaksVector)
    std::cout << (i++) << "\t" << p.i << "\t" << p.j << "\t"
              << std::setprecision(4) << std::fixed << p.val << "\t" << p.valid
              << std::endl;
}
...
void PivPoint::printDisps() {
  ...
  for (auto &d : m_dispsVec)
    std::cout << (i++) << "\t" << std::setprecision(4) << std::fixed
              << d.get_u() << "\t" << d.get_v() << "\t" << d.is_valid()
              << std::endl;
}

void PivPoint::printToOfstream(std::ofstream &file) {
  if (m_dispsVec[0].is_valid()) {
    file << i << "\t" << j << "\t" << m_dispsVec[0].get_u() << "\t"
         << m_dispsVec[0].get_v() << "\t" << std::endl;
  }
}

void PivPoint::set_xCoord(int x) { i = x; }

void PivPoint::set_yCoord(int y) { j = y; }
...
int PivPoint::x() { return i; }

int PivPoint::y() { return j; }
