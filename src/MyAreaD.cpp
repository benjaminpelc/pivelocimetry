...
MyAreaD::MyAreaD(BaseObjectType *cobject,
...
void MyAreaD::init(VecPivVector pv, double const max_vel) {
  ...
  std::cout << "Max velocity magnitude:\t" << max_velocity_magnitude
            << std::endl;
}
...
void MyAreaD::plot_area_box(CContextRptr cr) {
  ...
  cr->rectangle(0.0, 0.0, 1.0 - 2.0 * axis_padding_x,
                1.0 - 2.0 * axis_padding_y);
  cr->fill();
  cr->stroke();
}

void MyAreaD::add_vector_graphic(CContextRptr cr, PivEng::PivVector &p,
